...
// ─── Hand SVG ─────────────────────────────────────────────────────────────────
function HandDiagram({ hand, registeredIndexes, selectedIndex, onSelect, isDark }) {
    const coords = hand === "right"
        ? [
            { fi: 0, cx: 72,  cy: 120 },
            { fi: 1, cx: 108, cy: 68  },
            { fi: 2, cx: 132, cy: 55  },
    ...
    return (
        <svg viewBox="0 0 200 180" className="w-full max-w-[200px]" style={{ userSelect: "none" }}>
            <ellipse cx="100" cy="148" rx="55" ry="30" fill={palmColor} stroke={palmStroke} strokeWidth="1.5" />
            {coords.map(({ fi, cx, cy }) => {
                ...
                const stroke = isSelected ? "#1d4ed8" : isRegistered ? "#059669" : (isDark ? "#4b5563" : "#d1d5db");
                return (
                    <g key={fi} onClick={() => onSelect(fi)} style={{ cursor: "pointer" }}>
                        <ellipse
                            cx={cx} cy={cy}
                            rx={13} ry={16}
                            fill={fill}
                        ...
                        {isRegistered && !isSelected && (
                            <circle cx={cx + 7} cy={cy - 9} r={5} fill="#10b981" stroke="white" strokeWidth={1} />
                        )}
                        {isSelected && (
    ...
    return (
        <svg viewBox="0 0 100 60" className="w-24">
            <path
                ...
                fill="none" stroke="#e5e7eb" strokeWidth={strokeW} strokeLinecap="round"
            />
            <path
                d={`M ${cx - r} ${cy} A ${r} ${r} 0 0 1 ${cx + r} ${cy}`}
...
export default function RegisterFingerprint({ tableData }) {
    ...
    const selectedFingerData = useMemo(() =>
        currentFingerprints.find(f => f.finger_index === selectedFinger) ?? null,
    ...
    const handleCapture = useCallback(async () => {
        ...
        try {
            ...
            if (!res.data.success) throw new Error(res.data.message ?? "Capture failed");
            setCapturedData(res.data.data);
            setCaptureState("success");
        } catch (e) {
            setCaptureError(e.response?.data?.message ?? e.message ?? "Capture failed");
    ...
    const handleSave = useCallback(async () => {
        ...
        try {
            ...
            setEmpFingerprints(prev => {
                ...
                const existing = (prev[empId] ?? []).filter(f => f.finger_index !== selectedFinger);
                return {
                    ...prev,
                    [empId]: [
                        ...existing,
            ...
            setSaveMsg({ type: "success", text: "Fingerprint saved successfully." });
            setCapturedData(null);
            setCaptureState("idle");
        } catch (e) {
    ...
    const handleDelete = useCallback(async (fingerIndex) => {
        if (!selectedEmp) return;
        try {
            const res = await axios.delete(route("register-fingerprint.destroy"), {
                data: {
                    employid:     String(selectedEmp.employee_id),
            ...
            setEmpFingerprints(prev => {
                ...
                return {
                    ...prev,
                    [empId]: (prev[empId] ?? []).filter(f => f.finger_index !== fingerIndex),
                };
            });
    ...
    const fingerLabel = FINGERS.find(f => f.index === selectedFinger)?.label ?? "";
                    ...
                    <div className="p-6">
                        {filteredEmployees.length === 0 ? (
                            <div className="flex flex-col items-center justify-center py-20 text-base-content opacity-30 gap-3">
                                <FingerprintIcon style={{ width: 56, height: 56 }} />
                                <p className="text-sm font-medium">No employees found</p>
                            </div>
                        ) : (
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
                                {filteredEmployees.map(emp => {
                                    const empFps = empFingerprints[String(emp.employee_id)] ?? [];
                                        ...
                                        >
                                            <div className="card-body p-4 items-center text-center gap-2">
                                                ...
                                                <span style={{
                                                    ...
                                                    fontSize: 10,
                                                    fontWeight: 600,
                                                    background: fpCount > 0 ? "#d1fae5" : "#f3f4f6",
                                                    color: fpCount > 0 ? "#065f46" : "#6b7280",
                                                    border: `1px solid ${fpCount > 0 ? "#6ee7b7" : "#e5e7eb"}`,
                                                    whiteSpace: "nowrap",
                                                }}>
                                                    {fpCount > 0 ? `${fpCount}/10 fingers` : "No fingerprints"}
                                                </span>
                                            </div>
                                        </div>
                                    );
            ...
            {isModalOpen && selectedEmp && (
                <div className="modal modal-open">
                    <div className="modal-box max-w-3xl">
                        ...
                        {/* Save message */}
                        {saveMsg && (
                            <div className={`alert alert-sm py-2 mb-4 ${saveMsg.type === "success" ? "alert-success" : "alert-error"}`}>
                                {saveMsg.type === "success" ? <CheckCircleFilled /> : <CloseCircleFilled />}
                                <span className="text-sm">{saveMsg.text}</span>
                                <button className="btn btn-ghost btn-xs ml-auto" onClick={() => setSaveMsg(null)}>✕</button>
                            </div>
                        )}
                                    ...
                                    <div className="card-body p-5 gap-4">
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p className="text-xs opacity-50 font-mono">{selectedEmp.employee_id}</p>
                                            </div>
                                        </div>

                                        {selectedEmp.job && (
                                            <div className="bg-base-200 rounded-lg px-3 py-2">
                                                <p className="text-sm font-medium text-center text-base-content">{selectedEmp.job}</p>
                                            </div>
                                        )}

                                        {/* Stats */}
                                        <div className="flex items-center justify-between py-2 border-b border-base-300">
                                            <span className="text-xs opacity-70 font-medium">Registered</span>
                                            <span style={{
                                                display: "inline-flex",
                                                alignItems: "center",
                                                padding: "2px 8px",
                                                ...
                                                fontSize: 11,
                                                fontWeight: 600,
                                                background: currentFingerprints.length > 0 ? "#d1fae5" : "#f3f4f6",
                                                color: currentFingerprints.length > 0 ? "#065f46" : "#6b7280",
                                                border: `1px solid ${currentFingerprints.length > 0 ? "#6ee7b7" : "#e5e7eb"}`,
                                            }}>
                                                {currentFingerprints.length} / 10
                                            </span>
                                        </div>

                                        <div className="flex items-center justify-between py-1 border-b border-base-300">
                                            <span className="text-xs opacity-70 font-medium">Selected Finger</span>
                                            <span className="text-sm font-semibold text-base-content">{fingerLabel}</span>
                                        </div>

                                        {selectedFingerData && (
                                            <div className="flex items-center justify-between py-1 border-b border-base-300">
                                                <span className="text-xs opacity-70 font-medium">Status</span>
                                                <span style={{
                                                    display: "inline-flex",
                                                    alignItems: "center",
                                                    padding: "2px 8px",
                                                    ...
                                                    fontSize: 11,
                                                    fontWeight: 600,
                                                    background: selectedFingerData.is_active ? "#d1fae5" : "#f3f4f6",
                                                    color: selectedFingerData.is_active ? "#065f46" : "#6b7280",
                                                    border: `1px solid ${selectedFingerData.is_active ? "#6ee7b7" : "#e5e7eb"}`,
                                                }}>
                                                    {selectedFingerData.is_active ? "Active" : "Inactive"}
                                                </span>
                                            </div>
                                        )}
                                        ...
                                        <div className="grid grid-cols-2 gap-2">
                                            <div>
                                                <p className="text-[10px] text-base-content opacity-40 text-center mb-1">Right Hand</p>
                                                <HandDiagram
                                                    hand="right"
                                            ...
                                            <div>
                                                <p className="text-[10px] text-base-content opacity-40 text-center mb-1">Left Hand</p>
                                                <HandDiagram
                                                    hand="left"
                                                    registeredIndexes={registeredIndexes}
                                                    selectedIndex={selectedFinger}
                                        ...
                                        {currentFingerprints.length > 0 && (
                                            <div>
                                                <p className="text-[10px] text-base-content opacity-40 font-medium mb-1">Registered fingers:</p>
                                                <ul className="space-y-1">
                                                    {currentFingerprints
                                                        .sort((a, b) => a.finger_index - b.finger_index)
                                                        .map(fp => (
                                                            <li key={fp.finger_index} className="flex items-center justify-between gap-1">
                                                                <button
                                                                    onClick={() => setSelectedFinger(fp.finger_index)}
                                                                ...
                                                                >
                                                                    {FINGERS[fp.finger_index]?.label}
                                                                </button>
                                                                <span className="font-mono text-[10px] shrink-0" style={{ color: QUALITY_COLOR(fp.quality) }}>
                                                                    Q:{fp.quality}
                                                                </span>
                                                                <button
                                                                    onClick={() => handleDelete(fp.finger_index)}
                            ...
                            <div className="flex-1 min-h-0 card bg-base-100 shadow-lg border border-base-300">
                                <div className="card-body p-5 flex flex-col items-center justify-between gap-4">
                                    ...
                                    <div className="flex-1 flex flex-col items-center justify-center w-full">
                                        <div className="relative">
                                            ...
                                            <div className="w-44 h-44 bg-base-200 rounded-xl flex flex-col items-center justify-center border-4 border-base-300 relative overflow-hidden gap-3">
                                                ...
                                                {captureState === "capturing" && (
                                                    <>
                                                        <FingerprintIcon className="text-primary opacity-30" style={{ width: 64, height: 64 }} />
                                                        {/* Scanning line */}
                                                        <div
                                                ...
                                                {captureState === "success" && capturedData?.image && (
                                                    <img
                                                ...
                                                {captureState === "success" && !capturedData?.image && (
                                                    <CheckCircleFilled className="text-success" style={{ fontSize: 48 }} />
                                                )}

                                                {captureState === "error" && (
                                                    <CloseCircleFilled className="text-error" style={{ fontSize: 48 }} />
                                                )}

                                                {captureState === "idle" && (
                                                    <FingerprintIcon className="text-base-content opacity-20" style={{ width: 64, height: 64 }} />
                                                )}
                                            </div>
                                        ...
                                        {captureState === "success" && capturedData && (
                                            <QualityArc quality={capturedData.quality} />
                                        )}
                                        ...
                                        <div className="mt-3 text-center">
                                            {saving ? (
                                                <>
                                                    <p className="text-info font-medium text-base">💾 Saving...</p>
                                                    <p className="text-xs opacity-60 mt-1">Recording fingerprint data</p>
                                                </>
                                            ) : captureState === "success" ? (
                                                <>
                                                    <p className="text-success font-medium text-base">✓ Scan Successful!</p>
                                                    <p className="text-xs opacity-60 mt-1">Quality: {capturedData?.quality}</p>
                                                </>
                                            ) : captureState === "error" ? (
                                                <>
                                                    <p className="text-error font-medium text-base">Capture Failed</p>
                                                    <p className="text-xs opacity-60 mt-1 text-center px-2">{captureError}</p>
                                                </>
                                            ) : captureState === "capturing" ? (
                                                <>
                                                    <p className="font-medium text-base">Scanning...</p>
                                                    <p className="text-xs opacity-60 mt-1">Place finger on scanner</p>
                                                </>
                                            ) : (
                                                <>
                                                    <p className="font-medium text-base opacity-50">Ready</p>
                                                    <p className="text-xs opacity-40 mt-1">Press "Capture" to scan</p>
                                                </>
                                            )}
                                        </div>
                                    </div>
                                    ...
                                    <div className="w-full">
                                        <p className="text-[10px] text-base-content opacity-40 mb-1">Select finger:</p>
                                        <div className="grid grid-cols-5 gap-1">
                                            {FINGERS.map(f => {
                                                const isReg = registeredIndexes.has(f.index);
                                                const isSel = selectedFinger === f.index;
                                                return (
                                                    <button
                                                        key={f.index}
                                                        onClick={() => { setSelectedFinger(f.index); setCapturedData(null); setCaptureState("idle"); }}
                                                        title={f.label}
                                                        ...
                                                        style={{ fontSize: 10 }}
                                                    >
                                                        <div className="font-bold">{f.index}</div>
                                                        <div className="opacity-70 leading-tight text-[9px]">{f.label.replace("Right ", "R.").replace("Left ", "L.")}</div>
                                                    </button>
                                                );
                                            })}
                                        </div>
                                    ...
                                    <div className="flex gap-2 w-full">
                                        ...
                                        >
                                            {captureState === "capturing"
                                                ? <><LoadingOutlined spin /> Scanning…</>
                                                : <><ReloadOutlined /> {captureState === "success" ? "Re-capture" : "Capture"}</>}
                                        </button>
