...
export default function ScanLogs({ auth }) {
    ...
    // ── Scanner listener ──────────────────────────────────────────────────────
    const allEmployeesRef = React.useRef(allEmployees);
...
const handleQRDisplayScan = useCallback((code) => {
    const trimmed = code.replace(/^0+/, "").trim();
        if (trimmed.length < 1) return;
        const emp = selectedEmployeeForQRRef.current;
        if (!emp) return;
        const normalizedScanned = trimmed.toLowerCase();
        const normalizedEmpId   = String(emp.EMPLOYID ?? "").replace(/^0+/, "").toLowerCase().trim();
        if (normalizedScanned !== normalizedEmpId) {
    ...
    router.post(
        route("scan-logs.store"),
        {
            ...
            prodline: emp.PRODLINE,
            station: emp.STATION,
            log_type: qrLogTypeRef.current,
        },
        {
            preserveScroll: true,
            preserveState: true,
            onSuccess: () => {
                alert(`✓ Logged for ${emp.EMPNAME}`);
                setTimeout(closeQRDisplayModal, 1000);
            },
            onError: (e) => alert("Failed: " + JSON.stringify(e)),
            onFinish: () => setIsSaving(false),
        },
    ...
    useEffect(() => {
        fpPreselectedRef.current = fpPreselected;
    }, [fpPreselected]);
    ...
    //             route("scan-logs.fingerprint-identify"),
    //         );
    //         if (!res.data.success)
    ...
    //         if (
    //             pre &&
    //             emp.EMPLOYID.toLowerCase() !== pre.EMPLOYID.toLowerCase()
    //         ) {
    //             setFpState("mismatch");
    //             setFpError(
    //                 `Fingerprint belongs to ${emp.EMPNAME}, not ${pre.EMPNAME}.`,
    //             );
    //             return;
    //         }
    ...
    const handleFPScan = useCallback(async () => {
        ...
        setFpError(null);
        try {
            // Step 1: Capture on the client's machine (browser → SecuGen USB reader)
            const captured = await captureFingerprint();

            // Step 2: Send template to Laravel for matching against stored templates
            const res = await axios.post(
                route("scan-logs.fingerprint-identify"),
                {
    ...
    const handlePrintQR = async () => {
        ...
        setIsExporting(true);
        try {
            await exportEmployeeQRCodesToDocx(allEmployees, 6);
        } catch (e) {
            console.error(e);
    ...
    const handleQRCodeScan = () => {
        ...
        router.post(
            ...
            },
            {
                ...
                preserveState: true,
                onSuccess: () =>
                    alert(`✓ Logged for ${selectedEmployeeForQR.EMPNAME}`),
                onError: (e) => alert("Failed: " + JSON.stringify(e)),
                onFinish: () => setIsSaving(false),
            },
        );
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                            ...
                            <div className="flex gap-2 flex-wrap">
                                <button
                                    ...
                                    disabled={
                                        isExporting || !allEmployees.length
                                    }
                                >
                                    {isExporting ? (
                                        <>
                            ...
                            <div className="flex-1 card bg-base-100 shadow-lg border border-base-300">
                                <div className="card-body p-6 flex flex-col items-center justify-center">
                                    ...
                                    <ScanStatus
                                        ...
                                        successText={
                                            scannedEmployee
                                                ? `Employee found: ${scannedEmployee.EMPNAME}`
                                                : ""
                                        }
                                ...
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h2 className="text-lg font-bold text-base-content">
                                            {selectedEmployeeForQR.EMPNAME}
                                        </h2>
                                        <p className="text-xs opacity-50 font-mono">
                                            ID: {selectedEmployeeForQR.EMPLOYID}
                                        </p>
                                        <p className="text-xs opacity-50">
                                            {selectedEmployeeForQR.JOB_TITLE} ·{" "}
                                            {selectedEmployeeForQR.DEPARTMENT}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="flex flex-col items-center py-6 gap-6">
                            <div className="relative">
                                <div className="w-56 h-56 bg-base-100 rounded-2xl p-4 flex items-center justify-center border-4 border-base-300 shadow-xl">
                                    <QRCodeSVG
                                        value={selectedEmployeeForQR.EMPLOYID}
                                        size={200}
                                        level="H"
                                        includeMargin
                                        bgColor="transparent"
                                                    ...
                                                    <>
                                                        <div className="flex items-center justify-between mb-4">
                                                            <div className="flex items-center gap-3">
                                                                ...
                                                                >
                                                                    <UserOutlined
                                                                        className={`text-xl ${isPreview ? "opacity-40" : "text-primary"}`}
                                                                    />
                                                                </div>
                                                                <div>
                                                                    <h2
                                                                        className={`text-base font-bold ${isPreview ? "text-base-content opacity-50" : "text-base-content"}`}
                                                                    >
                                                                        {
                                                                            displayEmp.EMPNAME
                                                                        }
                                                            ...
                                                            <div
                                                                className={`w-8 h-8 rounded-lg flex items-center justify-center ${isPreview ? "bg-base-200" : "bg-primary/10"}`}
                                                            >
                                                                <IdcardOutlined
                                                                    className={
                                                                        isPreview
                                                        ...
                                                        <div className="bg-base-200 rounded-lg px-3 py-2 mb-4">
                                                            <p
                                                                className={`text-sm font-medium text-center ${isPreview ? "opacity-40" : "text-base-content"}`}
                                                            >
                                                                {
                                                        ...
                                                        <div className="space-y-2">
                                                            <div className="flex items-center justify-between py-2 border-b border-base-300">
                                                                ...
                                                                <span
                                                                    className={`text-sm font-medium text-right ${isPreview ? "opacity-40" : "text-base-content"}`}
                                                                >
                                                                    {
                                    ...
                                    <div className="mt-6 text-center">
                                        ...
                                        {fpState === "scanning" && (
                                            <>
                                                ...
                                                <p className="text-sm opacity-60">
                                                    {fpPreselected
                                                        ? `Verifying ${fpPreselected.EMPNAME}`
                                        ...
                                        {fpState === "mismatch" && (
                                            <>
                                                ...
                                                <p className="text-sm text-error opacity-70 mt-1 leading-snug">
                                                    {fpError}
                                                </p>
                                            </>
                                        )}
            ...
            <div className="grid grid-cols-2 gap-2">
                {logTypeOptions.map((opt) => (
                    ...
                    >
                        <span
                            className={
                                value === opt.value ? "text-white" : opt.color
                            }
                        >
                            {opt.label}
                        </span>
        ...
        <div className="mt-6 text-center">
            {isSaving ? (
                ...
                <>
                    ...
                    <p className="text-sm opacity-60">{successText}</p>
                </>
            ) : code ? (
                <>
                    <p className="text-error font-medium">Employee Not Found</p>
                    <p className="text-sm opacity-60">Code: {code}</p>
                </>
            ) : (
                <>
                    <p className="font-medium">Scanning…</p>
                    <p className="text-sm opacity-60">{idleText}</p>
                </>
            )}
        </div>
    );
